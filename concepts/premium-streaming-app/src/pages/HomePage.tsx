    ...
    <div className="relative h-[420px] rounded-2xl overflow-hidden group">
      ...
      <div className="relative h-full max-w-7xl mx-auto px-8 flex items-center">
        <div className="max-w-xl">
          ...
          {item.description && (
            <p className="text-text-muted text-sm mb-4 line-clamp-3" dangerouslySetInnerHTML={{ __html: sanitizeHtml(item.description || '') }} />
          )}
  ...
  return (
    ...
    >
      <div className="aspect-[3/4] relative">
        ...
        <div className="absolute top-2 left-2 bg-primary/90 px-2 py-1 rounded-md text-xs font-bold text-background z-[3]">
          EP {schedule.episode}
        </div>
        <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-sm px-2 py-1 rounded-md text-xs text-white/80 z-[3]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2">
          ...
          <p className="text-xs text-text-muted mt-1">{schedule.media.format || 'TV'}</p>
        </div>
      </div>
    </div>
  );
    ...
    <section className="mb-10">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {recent.map((entry) => (
          <div
            key={entry.id}
            onClick={() => navigate(`/search?q=${encodeURIComponent(entry.animeTitle.toLowerCase())}`)}
            className="group flex-shrink-0 w-[170px] cursor-pointer"
          >
    ...
    <section className="mb-10">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {recent.map((fav) => (
          <div
            key={fav.url}
            onClick={() => navigate(`/search?q=${encodeURIComponent(fav.title.toLowerCase())}`)}
            className="group flex-shrink-0 w-[170px] cursor-pointer"
          >
            <div className="aspect-[3/4] rounded-lg overflow-hidden bg-surface relative">
    ...
    <div className="flex gap-3 overflow-x-auto pb-2">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex-shrink-0">
          <SkeletonCard className="w-[170px] h-[255px]" />
        </div>
      ))}
    </div>
  );
            ...
            {recentLoading ? (
              <div className="flex gap-3 overflow-hidden">
                {Array.from({ length: 6 }).map((_, i) => (
                  <SkeletonCard key={i} className="w-[170px] h-[255px] flex-shrink-0" />
                ))}
              </div>
            ) : (
              <div className="relative">
                ...
                >
                  {dedupedRecent.map((s) => (
                    <div key={`${s.media.id}-${s.episode}`} className="flex-shrink-0 w-[170px]">
              ...
              <div className="relative">
                ...
                >
                  {currentSeason?.filter(m => !isHentai(m.genres)).map((media) => (
                    <div key={media.id} className="flex-shrink-0 w-[170px]">
