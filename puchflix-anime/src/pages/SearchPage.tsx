...
export function SearchPage() {
  ...
  // Build anilistMap for AnimeCard navigation (anilistId → AniList data)
  ...
  // Proxy all cover images so they load via our backend cache
  ...
  // 1. Map coverImage URLs through proxy
  const withProxiedCover = coverMap ? applyCoverMap(results || [], coverMap) : (results || []);
  // 2. Promote anilistCoverImage → image only when mapped to proxy URL
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-surface rounded-xl border border-border text-text placeholder:text-text-muted focus:outline-none focus:border-primary text-lg"
            />
          </div>
          <select
            value={domain}
            onChange={handleDomainChange}
            className="px-4 py-3 bg-surface rounded-xl border border-border text-text focus:outline-none focus:border-primary min-w-[160px]"
          >
